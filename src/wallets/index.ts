...
const mergeWalletData = (walletKitWallet: any, overrides: any): Wallet => {
  return merge(
    {
      id: walletKitWallet.name,
      name: walletKitWallet.name,
      isInstalled: walletKitWallet.installed ?? false,
      logoUrl: walletKitWallet.iconUrl,
      downloadUrls: walletKitWallet.downloadUrl,
    },
    overrides ?? {}
  ) as Wallet;
};

const sortWallets = (wallets: Wallet[], priorityIds: string[]): Wallet[] => {
...
export const useListWallets = () => {
  ...
  const allWallets = [
    ...walletKitWallets,
    ...customWallets.filter(
      (cw) => !walletKitWallets.find((w) => w.id === cw.id)
    ),
  ];

  const installedWallets = allWallets.filter((w) => w.isInstalled);
  const sortedInstalledWallets = sortWallets(
    installedWallets,
    PRIORITY_WALLET_IDS
  );

  const notInstalledPriorityWallets = PRIORITY_WALLET_IDS.map((id) =>
    allWallets.find((w) => w.id === id)
  ).filter((w): w is Wallet => w !== undefined && !w.isInstalled);
  ...
  const otherWallets = allWallets.filter(
    (w) => !mainWallets.find((mw) => mw.id === w.id)
  );
